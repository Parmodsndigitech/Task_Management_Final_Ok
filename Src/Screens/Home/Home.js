...
useEffect(() => {
  ...
  const todayTasks = data.filter(
    task =>
      task.taskDate.slice(0, 10) === today || task.dueDate.slice(0, 10) === today
  );
...
const handleDayPress = (day) => {
  ...
  const filtered = (data && Array.isArray(data)) ? data.filter(task => {
...
{
  dottedLoader&&
  <View style={{flex:1,backgroundColor:'rgba(0,0,0,0.8)',justifyContent:'center',alignItems:'center',position:'absolute',width:'100%',height:'100%',zIndex:999}}>
